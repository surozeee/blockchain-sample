...
describe("Functions / Libraries", function () {
  let contract;

  beforeEach(async () => {
    const MathLibrary = await ethers.getContractFactory(
      "contracts/functions/23_libraries/MathLibrary.sol:MathLibrary"
    );
    mathLibrary = await MathLibrary.deploy();
    await mathLibrary.deployed();

    const Contract = await ethers.getContractFactory(
      "contracts/functions/23_libraries/MyContract.sol:MyContract",
      {
        libraries: {
